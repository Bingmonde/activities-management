...
class MyAPI extends SimpleBaseService {
     ...
     async execute() {
        switch (this.req.method) {
            case "GET":
                ...
                await this.meeting_get();
                break;
            case "POST":
                console.log("enter post function");
                await this.meeting_post();
                break;
            case "PUT":
            await this.meeting_put();
                break;
            case "DELETE":
                await this.meeting_delete();
                break;
            default:
     ...
     async meeting_post() {
        ...
        if (urlElement.length === 2)        {
            ...
            let meeting_num = await this.myDB.queryDB(
                {
                    table: 'meeting',
            ...
            await this.myDB.insertDB('meeting', [
                {
    ...
    async meeting_put(){
        ...
        // update meeting
        const result = await this.myDB.queryDBRaw(
            `UPDATE \`${this.myDB.dataset}.meeting\`
            SET 
                title = \`${meeting.title}\`,
                describe = \`${meeting.describe}\`,
                location = \`${meeting.location}\`,
        ...
        if (urlElement.length == 3 ){
            ...
            try {
                await this.myDB.queryDBRaw(
                    `DELETE FROM \`${this.myDB.dataset}.meeting\`
    ...
    async getMeetingBySharedLink(code: string){
        //console.log('code', code);
        let id = parseInt(code.split('-')[1]);
        //console.log('id', id);
        const oneMeeting = await this.myDB.queryDB(
            {
                table: 'meeting',
        ...
        const dates = await this.myDB.queryDBRaw(
            `SELECT date_id, d_start, d_end, count(date_part_id) AS vote
        ...
        if (oneMeeting.length === 1) {
            ...
            if (oneMeeting[0].editor_link == code ){
                ...
                const participants = await this.myDB.queryDBRaw(
                    `SELECT part_id, CONCAT(CONCAT(first_name, ' ') , last_name) AS name, date_id 
    ...
    async getMeetingByID(username: string, id: string){
    ...
    async addingDates(meeting_id: number, dates: any){
         // about adding dates
         let date_num = await this.myDB.queryDB(
            {
                table: 'date',
        ...
        // save in date table
        await this.myDB.insertDB('date', dates);
        
        // save in intersection table for date and meeting
        let meet_date_num = await this.myDB.queryDB(
            {
                table: 'meet_date',
    ...
    // function utility
    generateRandomShareCode(): string{
